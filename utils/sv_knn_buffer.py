...
class SVKNNBuffer(Buffer):
    def __init__(self, args, episodic_mem_size, in_dim, out_dim, eps_mem_batch):
        ...
        self.input_batch_size = self.args.batch
        self.max_num_samples = self.args.max_num_samples
        self.num_k = self.args.num_k
        self.dist = self.args.dist
        self.is_adversarial_sv = self.args.is_adversarial_sv
        if self.args.is_adversarial_sv or self.args.is_dist_retrieval:
            self.adversarial_type = self.args.adversarial_type
            self.adv_coeff = self.args.adv_coeff

        self.is_penal = self.args.is_penalized_curr_task
        self.is_offline = self.args.is_offline
        self.is_accum_curr = self.args.is_accum_curr
        self.is_mir = self.args.is_mir
        ...
        if self.is_accum_curr:
            ...
            self.accum_size = self.episodic_mem_size // self.out_dim * 2 # assuming split task with 2 classes in a task
            # else:
