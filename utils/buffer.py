...
class Buffer:
    def __init__(self, args, episodic_mem_size, in_dim, out_dim, eps_mem_batch):
        ...
        self.episodic_images = np.zeros([episodic_mem_size] + in_dim)
    ...
    def get_mem(self, model, sess, current_x=None, current_y=None, exclude=None):
        ...
        if mem_filled_so_far < self.eps_mem_batch:
            ...
            if self.args.model in RANDOM_MEM_RETRIEVE and not self.args.is_mir:
                ...
                feed_dict_subsample = {model.x: subsample_x, model.y_: subsample_y}
                ...
                sess.run(model.save_weights)
                if model.args.optimizer == 'Adam':
                    sess.run(model.adam_save)
                feed_dict_current = {model.x: current_x, model.y_: current_y}
                if 'resnet' in self.args.arch:
                    feed_dict_current.update({model.train_phase: True})
                sess.run(model.train_step, feed_dict=feed_dict_current)
...
class GSS_Buffer(Buffer):
    def __init__(self, args, episodic_mem_size, in_dim, out_dim, eps_mem_batch):
        super(GSS_Buffer, self).__init__(args, episodic_mem_size, in_dim, out_dim, eps_mem_batch)

        self.max_num_sample_grad = self.args.num_sample_grad
    ...
    def update_mem(self, sess, model, batch_x, batch_y, task_id):
        ...
        if (self.examples_seen_so_far < self.episodic_mem_size):
    ...
    def get_grad_vec(self, sess, model, images, labels, single=False):
        for ii, (xx, yy) in enumerate(zip(images, labels)):
            if (single) or xx.ndim == 1:
                xx = [xx]
                yy = [yy]
            feed_dict = {model.x: xx, model.y_: yy}
