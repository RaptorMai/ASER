...
def _residual_block_first(x, out_channels, strides, trainable_vars, train_phase, apply_relu=True, name="unit", is_ATT_DATASET=False, pretrained=False):
    """
    ...
    with tf.variable_scope(name) as scope:
        ...
        if in_channels == out_channels:
            ...
            if not is_ATT_DATASET and not pretrained:
...
def load(saver, sess, ckpt_path):
    """
    Load pre-trained weights from ckpt_path.
    :param saver:
    :param sess:
    :param ckpt_path:
    """
...
def load_resnet18():
    """
    """
    dir_modeldict = './utils/saved_model/model.dict'
    dir_converter = './utils/saved_model/converter.dict'
    print("Load pre-trained weights")
    if not path.exists(dir_modeldict) or not path.exists(dir_converter):
        raise FileNotFoundError("No saved model parameters")

    else:
        # if model parameter dictionary exists, load and assign
        with open(dir_modeldict, 'rb') as f:
            saved_param = pickle.load(f)
        with open(dir_converter, 'rb') as f:
            converter = pickle.load(f)

        return saved_param, converter
